...
<script>
    ...
    function showTable() {
        ...
        var url = '{!!  route('member.ticket-agent.refreshCount') !!}';

        $.easyAjax({
            type: 'POST',
            url: url,
            data: {'startDate': startDate, 'endDate':endDate,'status':status,'priority':priority,'channelId':channelId,'typeId':typeId,'_token': '{{ csrf_token() }}' },
        ...
        //refresh datatable
        var url2 = '{!!  route('member.ticket-agent.data') !!}';
    ...
    $('body').on('click', '.sa-params', function () {
        ...
        swal({
            ...
            if (isConfirm) {
                ...
                var url = "{{ route('member.ticket-agent.destroy',':id') }}";
