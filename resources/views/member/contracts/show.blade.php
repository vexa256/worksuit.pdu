                ...
                <div class="col-xs-12" id="estimates">
                    <div class="row m-b-20">
                        <div class="col-xs-12">
                            ...
                            <a href="{{ route("member.contracts.download", $contract->id) }}" class="btn btn-default pull-right m-r-10"><i class="fa fa-file-pdf-o"></i> @lang('app.download')</a>
                            <div class="clearfix"></div>
                        </div>
                    </div>
                    <div class="card">
...
<script>
    ...
    function addDiscussion() {
        $.easyAjax({
            type:'POST',
            url:'{{route('member.contracts.add-discussion', $contract->id)}}',
            container:'#estimates',
    ...
    //Accept estimate
    function sign() {
        var url = '{{ route('member.contracts.sign-modal', $contract->id) }}';
        $.ajaxModal('#estimateAccept', url);
    }

    //Accept estimate
    function edit(id) {
        var url = '{{ route('member.contracts.edit-discussion', ':id') }}';
    ...
    $('body').on('click', '.remove-discussion', function(){
        ...
        swal({
            ...
            if (isConfirm) {
                ...
                var url = "{{ route('member.contracts.remove-discussion',':id') }}";
                url = url.replace(':id', id);
