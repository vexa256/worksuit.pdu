            ...
            <div class="white-box">
                <div class="row">
                    <div class="col-md-12 p-r-0">
                        <nav>
                            <ul>
                                <li class="tab-current"><a href="{{ route('member.invoice-recurring.show', $invoice->id) }}"><span>@lang('app.invoiceRecurring') @lang('app.info')</span></a>
                                </li>
                                <li ><a href="{{ route('member.invoice-recurring.recurring-invoice', $invoice->id) }}"><span>@lang('app.recurringInvoices')</span></a></li>
                            </ul>
                        </nav>
                    </div>
                </div>
                        ...
                        <div class="ribbon-content ">
                            ...
                            <div class="row">
                                ...
                                <div class="col-xs-6">
                                    ...
                                    <p class="text-muted">{{ $invoice->currency->currency_symbol.''.$invoice->recurrings->sum('total') }}</p>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-xs-6 b-r">
...
<script>
    ...
    function deleteAppliedCredit(invoice_id, id) {
        let url = '{{ route('member.all-invoices.delete-applied-credit', [':id']) }}';
    ...
    $(function () {
        var table = $('#invoices-table').dataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '{{ route('member.all-invoices.create') }}',
            deferRender: true,
            "order": [[0, "desc"]],
            language: {
        ...
        $('body').on('click', '.verify', function() {
            ...
            var url = '{{ route('member.all-invoices.payment-verify', ':id') }}'
            url = url.replace(':id', id);
