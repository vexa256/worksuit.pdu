        ...
        return datatables()
            ...
            ->addColumn('action', function ($row) use ($firstInvoice) {
                ...
                if ($row->credit_note == 0 && $row->status != 'draft'  && $row->status != 'canceled') {
                    if ($row->status == 'paid') {
                        $action .= '<li><a href="' . route('member.all-credit-notes.convert-invoice', $row->id) . '" data-toggle="tooltip"  data-invoice-id="' . $row->id . '" class="addCreditNote"><i class="fa fa-plus"></i> ' . __('modules.credit-notes.addCreditNote') . '</a></li>';
                    } else {
