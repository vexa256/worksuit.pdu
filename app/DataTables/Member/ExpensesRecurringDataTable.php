        ...
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                ...
                <ul role="menu" class="dropdown-menu pull-right">
                    <li><a href="' . route("member.expenses-recurring.show", $row->id) . '"><i class="fa fa-search" aria-hidden="true"></i> ' . trans('app.view') . '</a></li>
                    <li><a href="' . route("member.expenses-recurring.edit", $row->id) . '"><i class="fa fa-pencil" aria-hidden="true"></i> ' . trans('app.edit') . '</a></li>
            ...
            ->editColumn('user_id', function ($row) {
                return '<a href="' . route('member.employees.show', $row->user_id) . '">' . ucwords($row->name) . '</a>';
            })
            ...
            ->editColumn('item_name', function ($row) {
                return  '<a href="' . route("member.expenses-recurring.show", $row->id) . '" data-expense-id="' . $row->id . '" >' . ucfirst($row->item_name) . '</a>';
            })
            ->editColumn(
                'created_at',
                function ($row) {
