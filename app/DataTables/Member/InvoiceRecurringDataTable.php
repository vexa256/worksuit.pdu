...
class InvoiceRecurringDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                $action = '<a href="' . route('member.invoice-recurring.show', $row->id) . '"><i class="fa fa-search"></i>' . __('app.view') . '</a>';
            ...
            ->addColumn('action', function ($row)  {
                ...
                $action .= '<li><a href="' . route("member.invoice-recurring.show", $row->id) . '"><i class="fa fa-search"></i> ' . __('app.view') . '</a></li>';
                $action .= '<li><a href="' . route("member.invoice-recurring.edit", $row->id) . '"><i class="fa fa-pencil"></i> ' . __('app.edit') . '</a></li>';
            ...
            ->editColumn('total', function ($row) {
                ...
                return '<div class="text-right">' . __('app.total') . ': ' . currency_position($row->total, $currencySymbol) . '</div>';
            })
            ->editColumn(
                'issue_date',
                function ($row) {
    ...
    protected function getColumns()
    {
        ...
        if(in_array('projects', $modules) ){
            $dsData = array_slice($dsData, 0, 3, true) +
                [            __('app.project')  => ['data' => 'project_name', 'name' => 'project.project_name']] +
                array_slice($dsData, 3, count($dsData) - 1, true) ;
        }
