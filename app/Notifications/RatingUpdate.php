...
class RatingUpdate extends BaseNotification
{
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
            ->subject(__('email.ratingUpdate.subject') . ' - ' . $this->rating->project->project_name . '!')
            ->greeting(__('email.hello') . ' ' . ucwords($notifiable->name) . '!')
            ->line(__('email.ratingUpdate.text') . ':-' . $this->rating->project->project_name . ':-')
    ...
    public function toSlack($notifiable)
    {
        ...
        if (count($notifiable->employee) > 0 && (!is_null($notifiable->employee[0]->slack_username) && ($notifiable->employee[0]->slack_username != ''))) {
            return (new SlackMessage())
                ...
                ->content('*' . __('email.ratingUpdate.text') . '*' . "\n" . $this->rating->project->project_name);
        }
        return (new SlackMessage())
            ->from(config('app.name'))
            ->image($slack->slack_logo_url)
    ...
    public function toOneSignal($notifiable)
    {
        return OneSignalMessage::create()
            ->subject(__('email.ratingUpdate.text'))
            ->body(ucfirst($this->rating->project->project_name));
    }
}
